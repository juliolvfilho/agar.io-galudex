...
function getKillerRanking(killCounter) {
    if (killCounter <= 0) return '';
    if (killCounter == 1) return 'First Blood';
    if (killCounter == 2) return 'Double Kill';
    if (killCounter == 3) return 'Triple Kill';
    if (killCounter == 4) return 'Killing Spree';
    if (killCounter == 5) return 'Mega Kill';
    if (killCounter == 6) return 'Ultra Kill';
    if (killCounter == 7) return 'Dominating';
    if (killCounter == 8) return 'Unstoppable';
    if (killCounter == 9) return 'Monster Kill';
    return 'Godlike';
}

function playKillerRankingSound(killerRanking) {
    var soundName = killerRanking.toLowerCase().replace(' ', '_');
    setTimeout( function() {
...
function drawCircle(centerX, centerY, radius, sides, prickly) {
    ...
    for (var i = 0; i < sides; i++) {
        ...
        x = centerX + _radius * Math.sin(theta);
        y = centerY + _radius * Math.cos(theta);
        graph.lineTo(x, y);
    }
...
function drawVirus(virus) {
    ...
    drawCircle(virus.x - player.x + global.screenWidth / 2,
        ...
        virus.radius, global.virusSides, true);
}
